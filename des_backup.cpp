...
// Pad binary string to multiple of 64 bits with zero padding
string pad_binary_string(const string &input)
{
    ...
    return input + string(padding_needed, '0');
}
...
public:
    ...
    string decrypt(const string &input)
    {
        ...
        for (int i = 15; i >= 0; i--)
        {
            ...
            // XOR with round key (reversed order)
            string xored = Xor(round_keys[i], right_expanded);
...
int main()
{
    ...
    else if (mode == 2)
    {
        ...
        for (size_t i = 0; i < ciphertext.length(); i += 64)
        {
            ...
            // Create DES object and decrypt
            DES des(roundKeys);
            string plaintext = des.decrypt(block);
            result += plaintext;
        }
    ...
    {
        // TripleDES encrypt (EDE: Encrypt-Decrypt-Encrypt)
        ...
        string ciphertext = des1.encrypt(plaintext);
        ciphertext = des2.decrypt(ciphertext);
        ciphertext = des3.encrypt(ciphertext);

        cout << ciphertext << endl;
    }
    else if (mode == 4)
    {
        // TripleDES decrypt (reverse of EDE)
        ...
        // TripleDES reverse EDE: Decrypt with K3, Encrypt with K2, Decrypt with K1
        DES des1(roundKeys1);
        DES des2(roundKeys2);
        DES des3(roundKeys3);

        string plaintext = des3.decrypt(ciphertext);
        plaintext = des2.encrypt(plaintext);
        plaintext = des1.decrypt(plaintext);

        cout << plaintext << endl;
    }
    else
    {
        cerr << "Mode không hợp lệ!" << endl;
        return 1;
    }
